...
)  # Z = (X-mu)/sigma where Z is standard normally distributed
noise_truth_2 = (
    jax.random.normal(subkey, shape=cmb_field.shape, dtype=jnp.float64) * nstd
)
...
data_1 = (
    hp.alm2map(
        hp.almxfl(cmb_alms, fl=hp.gauss_beam(fwhm=res, lmax=2 * c["nside"])),
        nside=c["nside"],
        lmax=2 * c["nside"],
    )
    + noise_truth_1
...
cmb_alms = hp.map2alm(np.asarray(cmb_field), lmax=2 * c["nside"], mmax=2 * c["nside"])
#%%
result_ps, result_alm = gibbs(
    iter=15, init_ps=TTCl, data=all_data, noise=nstd**2, nside=c["nside"]
)
